...
public class Control extends JFrame implements KeyListener {
	...
	//public int skillev=0;
/**
 * boolean to see if wizard crashed or lost or stoped
 */
	public boolean wizardCrashedorLost=false;
	public boolean wallfollowerCrashedorLost=false;
	public boolean specialargs=false;
	
	public boolean frontReliable= true;
	public boolean leftReliable= true;
	public boolean rightReliable= true;
	public boolean backReliable=true;
	...
	/**
	 * boolean to see if wizard argument is given
	 */
	public boolean Wizardmode= false;
	public boolean WallFollowermode= false;
	public boolean SmartWallFollowermode= false;
	public boolean SmartWizard=false;
	...
	public void handleCommandLineInput(String[] args) {
	    ...
	    if(specialargs) {
	    	if(sensors!= null) {
	    		char[] arr = sensors.toCharArray();
	    		System.out.println(arr);
	    		if(arr.length==4  && (arr[0]!='0' ||arr[0]!='1') && (arr[1]!='0' ||arr[1]!='1') && (arr[2]!='0' ||arr[2]!='1') && (arr[3]!='0' ||arr[3]!='1')) {
	    			if(arr[0]=='0') {
		    			frontReliable=false;
		    			LOGGER.fine("Front sensor unreliable");
		    		}
					if(arr[1]=='0') {
						leftReliable=false;
						LOGGER.fine("left sensor unreliable");
					}
					if(arr[2]=='0') {
						rightReliable=false;
						LOGGER.fine("right sensor unreliable");
					}
					if(arr[3]=='0') {
						backReliable=false;
						LOGGER.fine("back sensor unreliable");
					}
	    		}
	    		LOGGER.fine("Invalid sensor argument");
	 
	    	}
	    	if(drive!=null) {
	    		
	    	
	    	String msg = "switching to manual driver, No viable paramter in command line input: " + drive; // message for feedback
	    	switch (drive) {
	    	case "Wizard":
		    	msg = "Command line input detected: Wizard robot.";
//		        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
		        Wizardmode= true;
		        break;
		    case "SmartWizard":
		    	msg = "Command line input detected: SmartWizard robot.";
//		        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
		        SmartWizard= true;
		        break;
		    case "WallFollower":
		    	msg = "Command line input detected: WallFollower robot.";
//		        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
		        WallFollowermode= true;
		        break;
		    case "SmartWallFollower":
		    	msg = "Command line input detected: SmartWallFollower robot.";
//		        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
		        SmartWallFollowermode= true;
		        break;
		    case "Manual":
		    	msg = "Command line input detected: manual driver.";
//		        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
		        break;
	    	}
	    	LOGGER.fine(msg);
	    	...
	    	if(generation!=null) {
	    		String msg = "Error in handling command line input: " + generation; // message for feedback
	    		switch (generation) {
	    	    case "Prim" :
	    	    	...
	    	    	break;
	    	    case "Boruvka":
	    	    	msg = "Command line input detected: generating random maze with Boruvka algorithm.";
	    	    ...
	    	    default: // assume this is a filename
	    	    	try{File f = new File(generation) ;
	    	        if (f.exists() && f.canRead())
	    	        {
	    	            ...
	    	            msg = "Unknown 2() command line parameter: " + generation + " ignored, operating in default mode.";
	    	        }
	    	    	}catch(Exception e) {
	    	    		 msg = "Error in loading file for paramter : " + generation + " ignored, operating in default mode.";
	    	    	}
	    	    	break;
	    	    }
	    ...
	    case "Boruvka":
	        ...
	        break;
	    case "Wizard":
	    	msg = "Command line input detected: generating random maze with Wizard robot.";
	        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
	        Wizardmode= true;
	        break;
	    case "SmartWizard":
	    	msg = "Command line input detected: generating random maze with SmartWizard robot.";
	        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
	        SmartWizard= true;
	        break;
	    case "WallFollower":
	    	msg = "Command line input detected: generating random maze with WallFollower robot.";
	        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
	        WallFollowermode= true;
	        break;
	    case "SmartWallFollower":
	    	msg = "Command line input detected: generating random maze with SmartWallFollower robot.";
	        ((StateTitle)currentState).setBuilder(Order.Builder.DFS);
	        SmartWallFollowermode= true;
	        break;
