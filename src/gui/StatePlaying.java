...
import generation.ReliableRobot;
import generation.ReliableSensor;
import generation.SmartWallFollower;
import generation.SmartWizard;
import generation.UnreliableRobot;
import generation.UnreliableSensor;
import generation.WallFollower;
import generation.Wizard;
...
public class StatePlaying implements State {
    ...
    /**
     * boolean used to tell if wizard robot succesfully fufilled
     */
    public boolean wizardry=false;
    
    /**
     * integer used to past the robots travel distance for winning or losing screen
     */
    public int pathlengthy=0;
    
    public float batterylevelset= 3500;
    ...
    public void start(Control controller, MazePanel panel) {
        ...
        if(control.Wizardmode) {
        	runWiz();
        }
        if(control.WallFollowermode) {
        	runWallfollower();
        }
        if(control.SmartWallFollowermode) {
        	runSmartWallfollower();
        }
        if(control.SmartWizard) {
        	runSmartWiz();
        }
        
    }
    /**
     * method that runs the wizard robot algorithm
     */
    
    protected void runWiz(){
    	
	    	showMaze = true ;
	        showSolution = true ;
	        mapMode = true;
        
	        control.wizardCrashedorLost= false;
    		Robot robo= new ReliableRobot();
    		robo.setController(control);

    		robo.setBatteryLevel(batterylevelset);
    		RobotDriver robodriver= new Wizard();
    		robodriver.setRobot(robo);
    		robodriver.setMaze(maze);
    		control.setRobotAndDriver(robo, robodriver);
    		...
    		try{
    			...
    			pathlengthy=robodriver.getPathLength();
    		
    		}catch(Exception e) { 
    			pathlengthy=robodriver.getPathLength();
    			control.wizardCrashedorLost= true;
    			switchFromPlayingToWinning(pathlengthy);
    			
    		}
    		
    	
    }
    protected void runSmartWiz(){
		...
		robo.setBatteryLevel(batterylevelset);
		RobotDriver robodriver= new SmartWizard();
		robodriver.setRobot(robo);
		robodriver.setMaze(maze);
		control.setRobotAndDriver(robo, robodriver);
		for(Direction i: Direction.values()) {
    ...
    protected void runWallfollower(){
        ...
        control.wallfollowerCrashedorLost= false;
		...
		try{
			...
			pathlengthy=robodriver.getPathLength();
			control.wallfollowerCrashedorLost= true;
			switchFromPlayingToWinning(pathlengthy);
    ...
    protected void runSmartWallfollower(){
