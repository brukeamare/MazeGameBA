...
public class StateWinning implements State {
    ...
    public float batterylevelinit=0;
    ...
    public void start(Control control, MazePanel panel) {
        ...
        /**
         * sees if wizard crashed and put on losing screen
         */
        if (control.WallFollowermode|| control.SmartWallFollowermode) {
        	Robot r=control.getRobot();
          batterylevel=r.getBatteryLevel();
          batterylevelinit= ((UnreliableRobot)r).batterylevelinit;
        }
        if (control.Wizardmode|| control.SmartWizard) {
        	Robot r=control.getRobot();
          batterylevel=r.getBatteryLevel();
          batterylevelinit= ((ReliableRobot)r).batterylevelinit;
        }
        
        
        if(control.wizardCrashedorLost||control.wallfollowerCrashedorLost) {
        	view.setControl(control);
        	view.setbattery(batterylevelinit);
        	view.redrawLosing(panel, pathLength, batterylevel );
        }
        else {
        view.setControl(control);
        view.setbattery(batterylevelinit);
        view.redrawFinish(panel, pathLength, batterylevel);}
