...
public class SimpleScreens {
	...
	/**setter method to call on control variables
	 * 
	...
	public void setbattery(float cont) {
		batterylevelinit= cont;
	}
    /**
	 ...
	 * new method for if the robot crashes or loses
	 * parameters used to create exit screen of robot info
	 * @param panel
	 * @param pathlength
	 * @param batterylevel
	 */

	void redrawLosing(MazePanel panel, int pathlength, float batterylevel) {
		Graphics g = panel.getBufferGraphics() ;
        ...
        if (null == g) {
        	...
        	redrawLosing(g, pathlength, batterylevel );
        }
	}
	/**
	 * Helper method for if the robot crashes or loses
	 * parameters used to create exit screen of robot info
	 ...
	 * @param batterylevel
	 */
	private void redrawLosing(Graphics gc, int pathlength, float batterylevel) {
		...
		updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
		centerString(gc, "RobotCrashed or Lost Power!", 160);
			
		updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
		centerString(gc, "Battery charge consumed: "+ (batterylevelinit-batterylevel), 220);
		updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
		centerString(gc, "Battery charge left: "+ batterylevel,240 );
		updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
		centerString(gc, "Robot Path length: " + pathlength , 260);
     ...
     * @param panel holds the graphics for the off-screen image
     */
	void redrawFinish(MazePanel panel,  int pathlength, float batterylevel) {
		Graphics g = panel.getBufferGraphics() ;
        ...
        if (null == g) {
            ...
            redrawFinish(g, pathlength, batterylevel);
        }
	}
	/**
	 * Helper method for redraw to draw final screen, screen is hard coded
	 * @param gc graphics is the off-screen image
	 */
	private void redrawFinish(Graphics gc,  int pathlength, float batterylevel) {
		// produce blue background
		drawBackground(gc);
		// write the title 
		updateFontAndColor(gc, largeBannerFont, MazeColors.TITLE_LARGE);
		...
		if(control.Wizardmode|| control.WallFollowermode|| control.SmartWallFollowermode||control.SmartWizard) {
		updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
		centerString(gc, "Battery charge consumed: "+ (batterylevelinit-batterylevel), 220);
		updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
		centerString(gc, "Battery charge left: "+ batterylevel,240 );
